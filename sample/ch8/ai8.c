...
void setstartch(char *startch,char *newline)
{
 ...
 char target[MAXLINE] ;//全角文字だけを取り出した入力文
 char line[MAXLINE] ;//処理対象の文（前回または前々回の文）
 int now,last ;
 int startpoint ;
 static char oldline[MAXLINE]="私は人工無能です．" ;

 /*返答の対象とする文の選択（前回または前々回）*/
 if(setrnd(3)<2) strncpy(line,newline,MAXLINE) ;//前回の文
 else strncpy(line,oldline,MAXLINE) ;//前々回の文
 ...
 if(strlen(startch)<LIMITLENGTH) strncpy(startch,"人工知能",MAXLINE);//短すぎる場合
 strncpy(oldline,newline,MAXLINE) ;//1回前の文を保存しておく
}
